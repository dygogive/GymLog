...
class InsertWorkoutGymDayUseCase @Inject constructor(
    private val repository: WorkoutRepositoryInterface
) {
    suspend operator fun invoke(workoutGymDay: WorkoutGymDay): Long {
        return repository.insertWorkoutGymDay(workoutGymDay)
    }
}